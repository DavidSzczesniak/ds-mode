...
3. Fan out across your chat history. Launch parallel fresh Explore workers per `../ds-mode/references/workers.md`, each taking a slice of the corpus. Tell every worker to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current session (put your own `$PI_SESSION_FILE` path in each brief) plus obvious noise (subagent, eval, and test sessions). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the session UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the workers. The main thread gets only their findings.
