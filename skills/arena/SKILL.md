...
4. Assign output paths. Each candidate writes to its own location (a git worktree where possible, otherwise `/tmp/arena-<slug>/candidate-<n>/`). N candidates writing to the same path is shared mutable state and fails the the [**separate-before-serializing-shared-state**](../principle-separate-before-serializing-shared-state/SKILL.md) principle skill test.
...
The synthesized artifact has to hold up under the same scrutiny as any other output, per the [**prove-it-works**](../principle-prove-it-works/SKILL.md) principle skill. The arena does not earn you a pass.
