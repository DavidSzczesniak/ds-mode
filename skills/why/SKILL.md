---
name: why
description: "Use for 'why does X work this way', 'why we picked Y', design rationale, regressions, postmortems, or data-backed thresholds. Discovers available evidence tools, searches source control and relevant evidence categories (issue tracker, long-form docs, real-time chat, infrastructure observability, error tracking, product analytics warehouse), then returns a cited read on decisions and tradeoffs. Use how for runtime behavior."
---
...
Historical context spreads across seven evidence categories: source control history, issue or ticket tracking, long-form documents, real-time team chat, infrastructure observability, error or exception tracking, and product analytics warehouses. You cannot predict from the question alone which one holds the answer, so the skill enumerates available evidence tools at run time, maps each to a category, searches source control and the categories that can materially answer the question, then synthesizes with explicit confidence calibration. Null results from searched categories are first-class evidence about how the decision was made; report them alongside positive findings. Record every unsearched category and why it was unavailable or skipped.
...
- **Hedge on purpose.** When evidence is indirect, your language should signal it ("appears to", "likely", "suggests"). Confidence-matching phrasing is a feature of the output, not a stylistic choice to override during synthesis.
...
Read `references/epistemics.md` for the full confidence framework and phrasing guide. The synthesis must follow it.
...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (supplied files, recent edits, current repository state, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.

## Step 2. Establish the Code Anchor

Before investigating the historical record, anchor the investigation in concrete code. You need:
...
Build this inline. It's cheap, and a complex investigation needs it.
...
Capture this as seed context (file paths, symbols, commits, PR numbers, linked ticket IDs). Pass it to the investigator for a complex question so it doesn't rediscover it.

## Step 3. Investigate the Evidence

Search source control for every investigation. Search another evidence category when the code anchor gives you a concrete lead into it or that category can materially answer the question. Account for every category in the final coverage map.

### Discovery

Inspect the available tool catalog for connected services and evidence sources.
...
Source control is always available through git and `gh`. For the other six, classify using the service name, instructions, tool names, and resource descriptors. If a tool could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map.

Aim for a complete **coverage map**, not a minimal one. A null result from an issue tracker is evidence the decision was not ticketed, a useful fact in itself. Document the null. For an unavailable or skipped category, record the reason instead.

Assess the investigation before delegating:

- **Bounded** (a narrow rationale question, a small commit history, or one substantive PR): investigate directly.
- **Complex** (history spanning many commits or evidence systems, a regression archaeology sweep, or a broad design decision): start one fresh investigator agent without inherited conversation history (`fork_turns: "none"`). Give it a read-only investigation task with no implementation or external writes.

The investigator gets:
1. The base prompt from `references/investigator-prompt.md`
2. The category playbooks `references/sources/<source>.md` for the selected evidence sources, adapted from the examples in `references/source-playbook.md`
...
Each entry lists what the category physically contains and the kind of "why" it uniquely surfaces. Use it to know what to search, how to name a gap when a category returns empty, and when a skip is justified. Every category overlaps, but each owns a kind of evidence the others cannot recover.
...
"It's pure feature code, error tracking won't have anything" is **not** sufficient, and neither is "I doubt long-form docs would have this." Run the search when the source can materially answer the question; let the null result speak. The cost of missing a design doc that actually exists is a wrong answer.

If your scope assessment suggests a single-commit trivial target where the PR description contains the complete answer, answer inline after completing the coverage map.

## Step 4. Synthesize

Synthesize the findings directly. Read `references/synthesizer-prompt.md` as the synthesis brief. Use:
1. The evidence gathered directly and any investigator findings, including null results and categories skipped with justification
2. The code anchor from Step 2 (file paths, symbols, commit hashes, PR numbers, ticket IDs)
3. The user's original question
4. The epistemics framework from `references/epistemics.md`
5. The output format from `references/synthesizer-prompt.md`

The final output is a confidence-weighted, evidence-cited narrative with clearly separated "what we know" and "what we're inferring" sections, plus honest acknowledgment of gaps and null-result sources.

## Step 5. Present

Present the synthesis to the user. You may edit for clarity or add context from the conversation, but **do not rewrite the confidence language**. The epistemic framing is the product. Dropping the hedges to sound more authoritative is the exact failure mode this skill exists to prevent.
...
**Sources Consulted**. One line per evidence category, including the ones that returned nothing. The reader should see at a glance (a) which tools were queried, (b) which came back empty, and (c) which were skipped and why. This coverage map lets the user judge breadth and redirect if something obvious was missed.
...
- **Skipping sources by anticipation**. Deciding up front that "long-form docs probably don't have this" or "this isn't an error tracking thing" when the category can materially answer the question. A null result is a data point; a skipped search is a blind spot unless its reason is recorded.
- **Unbounded investigation**. Searching every connected system without tying each query to the question or code anchor. The coverage map requires an explicit reason for every search and skip.

## Reference Files

- `references/epistemics.md`. Confidence tiers and phrasing guide. The synthesis must follow it.
- `references/investigator-prompt.md`. Base prompt template for the investigator agent.
- `references/source-playbook.md`. Index pointing at the category playbooks below.
- `references/sources/*.md`. One self-contained example playbook per category, plus cross-cutting `incident-postmortem.md`. Read the files that match the selected categories and adapt them to the available tools.
- `references/synthesizer-prompt.md`. Synthesis brief and output format for the active agent.
