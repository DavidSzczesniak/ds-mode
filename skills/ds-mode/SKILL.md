...
**Reserved decisions stay reserved.** A call the ticket or the operator leaves open ("for review", "confirm or reject", a product choice) is not yours, even under a full-autonomy grant. Keep the current behavior unless the ticket names a default, and list the call as an open decision in the reply and the PR body.
...
Launch every worker with a fresh context and a brief per `references/workers.md`. Workers report results and may keep their own task plans. They never share or mutate the lead's checklist.
