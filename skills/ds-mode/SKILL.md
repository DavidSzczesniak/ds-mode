...
- Docs, RFCs, readmes, PR descriptions, or commit messages → the [**technical-writing**](../technical-writing/SKILL.md) skill. Every commit you make gets a Conventional Commit title, `type(scope): outcome`. Append a ticket reference such as `(#123)` when the commit resolves one. Give it a body of two or three lines that says why. Merge commits and Pause safely's `wip:` commit are exempt. Before any push, stay in the checkout you are pushing and run `<this skill's directory>/scripts/check-commit-titles.sh <base-branch>` by its absolute path. Push only when it exits 0. **Titles and commit messages** in [Opening a PR](playbooks/opening-a-pr.md) has the details.
...
Launch every ordinary worker with a fresh context and a brief per `references/workers.md`. Do not spawn a worker from a brief you wrote that lacks SCOPE, FORBIDDEN, or VERIFY. Record the canonical child target returned by dispatch. Workers report results and may keep their own task plans. They never share or mutate the lead's checklist.
