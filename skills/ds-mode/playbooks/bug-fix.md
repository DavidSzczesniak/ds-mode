...
**You own this task. Plan, review, verify.** Delegate investigation and the fix to named Pi workers, stay in the lead.
...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a named Pi Implement worker with a specific scope; review the diff.
...
Investigation may fan out `how` and `why` to named Pi Explore workers with separate briefs.
