...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a fresh native Codex Implement child with a specific scope; review the diff.
...
6. Run **Opening a PR** only when the user requested a PR.

Investigation may fan out `how` and `why` to fresh native Codex Explore children with separate briefs.
