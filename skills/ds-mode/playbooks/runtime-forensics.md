...
1. Capture the live signal on the matching surface via [**control-cli**](../../control-cli/SKILL.md) or [**control-ui**](../../control-ui/SKILL.md): a CPU profile for a spinning process, a heap snapshot for a leak, a CDP trace for a visual glitch. A real artifact, not a guess.
2. Reduce the artifact to the smoking gun: the function on the hot path, the retainer chain from the leaked object to a GC root, the loop firing without input. Parse large artifacts in a fresh native Codex Explore child (the [**guard-the-context-window**](../../principle-guard-the-context-window/SKILL.md) principle skill), keep the reduced finding in the main thread.
