...
- [ ] At the head SHA the PR opens with and at each later push that changes the patch, run the swarm per [swarm](../../swarm/SKILL.md). One gates lane. The live lanes from the PR's **Verify, live** block. The perf lane from its **Verify, perf** block. Two or more audit lanes on the Review profile, each with its own focus, that read the diff and the verification evidence and distrust the PR body. The lead audits the verification evidence in the owner's report before the verdict.
...
Each live lane runs as a fresh worker on the swarm lane profile at the PR head and drives the surface through its own control-skill session. Drive through [control-ui](../../control-ui/SKILL.md) or [control-cli](../../control-cli/SKILL.md).
