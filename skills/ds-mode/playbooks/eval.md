...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's exact native transcript resolved from its canonical child target per `../references/workers.md`. Do not search unrelated sessions. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
