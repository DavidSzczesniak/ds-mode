...
- Comparing two variants: one Judgment worker scores both sets in a single pass on one scale, blind to which set each came from. Two review runs with different prompts don't compare, the calibration drifts.
...
4. **Spawn N parallel candidates** as named persistent Pi sessions per the **arena** skill's Phase B. Use isolated writable paths and the profiles in `../references/pi-profiles.md`. Same-family candidates test run-to-run variation, not model-family diversity.
...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's exact Pi JSONL from its recorded session ID. Do not search unrelated sessions. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
...
**Reply:** variant under test, rubric, per-candidate notes, same-family independent review, your synthesis, session IDs, and a recommendation for whether to promote the variant.
