...
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Run bounded lead-controlled iterations per component until the diff is zero or the evidence reaches a real dead end.
5. Run **Opening a PR** per component or safe batch only when the user requested PRs.
