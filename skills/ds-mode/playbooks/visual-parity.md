...
4. Verify each component against its baseline via image diff on the matching surface with [**control-ui**](../../control-ui/SKILL.md). A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Run bounded lead-controlled iterations per component until the diff is zero.
