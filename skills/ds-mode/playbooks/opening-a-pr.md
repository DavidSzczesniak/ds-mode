...
**Evidence.** Inspect the final base-to-head diff, commits, changed files, and validation results. Reuse inspection, cleanup, review, and validation from this task when their inputs have not changed. Before publishing, finish any checks the repository requires. Run [deslop](../../deslop/SKILL.md) over the diff before each commit. Run [no-comments](../../no-comments/SKILL.md) before review. Whoever opens the PR, lead or worker, first runs [interrogate](../../interrogate/SKILL.md) when the design was contested and the task has not run it yet, and confirms deslop and no-comments covered the final diff.
