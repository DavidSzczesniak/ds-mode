...
Use this reference when the PR Check and Triage playbook (`../playbooks/pr-check-and-triage.md`) handles Bugbot or review-automation comments. The goal is not to ignore Bugbot by default. The goal is to stop treating every comment as a required code change.
...
- Confidence: candidate
- Skip when: Bugbot flags an export, component, helper, or file as unused, and `gh pr view <number> --json baseRefName,headRefName`, upper-stack diffs, or PR context shows it is used by a later PR in the stack.
- Do not skip when: The current PR is not part of a stack, the symbol is public API, or the supposed dependent PR use cannot be verified.
- Example signal: "Exported component is never used" with a human reply like "used dependent PR".
