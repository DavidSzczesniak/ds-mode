...
const LANES = /(\w+) lanes at the PR head/;
...
const PROGRAM_MARKERS = ["explicit go", "zero-writes", "status message"];
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.test(live.rest)) fail(live.n, `${pr.title}: Verify, live lacks "<count> lanes at the PR head"`);
		...
		if (numbers.length < MIN_LANES || numbers.join(",") !== expected) fail(live.n, `${pr.title}: lanes are [${numbers.join(",")}], expected 1 to N with N at least ${MIN_LANES}`);
		const stated = live.rest.match(LANES);
		const count = stated ? (/^\d+$/.test(stated[1]) ? Number(stated[1]) : WORDS.indexOf(stated[1].toLowerCase())) : -1;
		if (stated && count !== numbers.length) fail(live.n, `${pr.title}: states ${stated[1]} lanes but lists ${numbers.length}`);
