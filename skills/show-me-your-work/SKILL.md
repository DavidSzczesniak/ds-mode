...
- Prefer evidence produced by committed scripts over hand-made one-offs (the [**encode-lessons-in-structure**](../principle-encode-lessons-in-structure/SKILL.md) principle skill).
...
Before handing back, launch a fresh Judgment worker per `../ds-mode/references/workers.md`, on a different model family from the one that did the work. The Judgment profile runs on Anthropic. When most of the work ran on Anthropic, use the second-opinion row for Anthropic work instead. Label the review per `../ds-mode/references/worker-profiles.md`. Self-review is not a substitute. The worker reads the audit trail and the run's session file, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
...
Every reply for a run that produced a trail ends with an "Attention" section. Lead with the reviewer's model and its review label on its own line (`reviewed by <model>, <label>`), then list each flag pointing to specific rows or moments. "No flags" is a valid value. The model name is not.
