...
Keep deliberate exclusions out of this review. Cloud agents, Graphite, Orchestrate, Autopilot, polling, and automatic merging are inactive design decisions. Shipping is active without Graphite: it verifies each PR and merges only on an explicit request. They are not failed dogfooding experiments.
